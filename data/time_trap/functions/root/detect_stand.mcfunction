#检测站在什么方块上面，-1代表普通，0到5分别是下界合金、钻石、绿宝石、金、铁块以及铜块
scoreboard players set @s tt_standOn -1

execute if entity @s[scores={tt_standOn=-1}] if block ~ ~-1 ~ minecraft:netherite_block run scoreboard players set @s tt_standOn 0
...
execute if entity @s[scores={tt_standOn=-1}] if block ~ ~-1 ~ minecraft:copper_block run scoreboard players set @s tt_standOn 5